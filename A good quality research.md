...
> Even when two adaptation problems are matched on (M\_1,\\ldots,M\_k), their sample complexity differs by a factor of at least four because of (Z), a property absent from current theories.
...
Formally, construct two cases (T\_1) and (T\_2) such that:



\[

M\_j(S,T\_1)=M\_j(S,T\_2)
...
where (M\_1,\\ldots,M\_k) are the strongest established measures or explanations.
...
> For every (m), there exist two learning problems that are identical under established quantities (M\_1,\\ldots,M\_k), but whose adaptation complexities differ by at least a factor (m).
...
EXPERIMENT\_OBJECTION\_MATRIX.md
...
M\_j(T\_1)=M\_j(T\_2)\\quad\\forall j,
...
> Manipulate (Z), match (M\_1,\\ldots,M\_k), and measure (Y).
